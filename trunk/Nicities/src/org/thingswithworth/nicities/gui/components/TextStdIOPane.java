...
public class TextStdIOPane extends ToolbaredTextArea {
	...
	private TextStdIOPane(JTextArea area) {
		...
		/*
		 * redirect = new PipedInputStream(); System.setIn(redirect); try {
		 * ((PipedInputStream) redirect).connect(redirectFrom); } catch
		 ...
		 * 
		 * @Override public void changedUpdate(DocumentEvent de) { try { String
		 * inserted = de.getDocument().getText(de.getOffset(), de.getLength());
		 * redirectFrom.write(inserted.getBytes()); } catch
		 * (BadLocationException e) { e.printStackTrace(); } catch (IOException
		 * e) { e.printStackTrace(); } }
		 * 
		 * @Override public void insertUpdate(DocumentEvent de) { try { String
		 * inserted = de.getDocument().getText(de.getOffset(), de.getLength());
		 * redirectFrom.write(inserted.getBytes()); } catch
		 * (BadLocationException e) { e.printStackTrace(); } catch (IOException
		 * e) { e.printStackTrace(); }
