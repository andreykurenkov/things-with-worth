...
/**
 * A converter which renders the internal tree node representation as specific HTML text, but which is easier to change in
 * behaviour than its superclass and has a noImages property, which can be set to leave out all images
 * 
 * Note: Credit goes to http://www.integratingstuff.com/2012/04/06/hook-into-wikipedia-using-java-and-the-mediawiki-api/
 */
public class ExtendedHtmlConverter extends HTMLConverter {
	...
	protected void renderContentToken(Appendable resultBuffer, ContentToken contentToken, IWikiModel model)
			throws IOException {
	...
	protected void renderHtmlTag(Appendable resultBuffer, HTMLTag htmlTag, IWikiModel model) throws IOException {
		htmlTag.renderHTML(this, resultBuffer, model);
	}

	protected void renderTagNode(Appendable resultBuffer, TagNode tagNode, IWikiModel model) throws IOException {
