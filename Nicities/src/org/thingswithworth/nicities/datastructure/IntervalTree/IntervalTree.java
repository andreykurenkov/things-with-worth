package org.thingswithworth.nicities.datastructure.IntervalTree;

import java.util.ArrayList;
import java.util.List;

/**
 * An Interval Tree is essentially a map from intervals to objects, which can be queried for all data associated with a
...
public class IntervalTree<Type> {
	 ...
	 * @return all intervals that contain time
	 */
	public List<Interval<Type>> getIntervals() {
		return intervalList;
	}
	...
	/**
	 * change an interval object to the interval tree's list Will not rebuild the tree until the next query or call to build
	 * 
	 * @param interval
	 *            the interval object to change
	 * @param changeStart
	 *            the amount by which to change the start
	 * @param endChange
	 *            the amount by which to change the end
	 */
	public void changeInterval(Interval<Type> interval, long startChange, long endChange) {
		int index = intervalList.indexOf(interval);
		if (index != -1) {
			Interval<Type> wantedInterval = intervalList.get(index);
			wantedInterval.setStart(startChange + wantedInterval.getStart());
			wantedInterval.setEnd(endChange + wantedInterval.getEnd());
			inSync = false;
		}
	}

	/**
	 * CHanges inSync to false.
	 */
	public void notifyNotInSync() {
		inSync = false;
	}
